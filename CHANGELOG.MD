...
- Lagt till `photoUrl` för `Hehe`, där en bild för tidningens framsida genereras automatiskt utifrån PDFen. Då används paketet `pdf-to-png-converter`
- Exponerat `uploadedAt` för `Hehe`, vilket är en `DateTime` för när Hehen laddades upp
- `DateTime` som en ny `Scalar`

### Ändrat

- `addHehe`-APIn så att denna dessutom skapar framsidan för tidningen och sparar motsvarande `photoUrl`
- Enhetstester för `Hehe` så att dessa är kompatibla med tilläggen ovan
