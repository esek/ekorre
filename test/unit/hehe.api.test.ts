...
beforeEach(async () => {
  // Delete all rows
  await knexInstance<DatabaseHehe>(HEHE_TABLE).delete().whereNotNull('number');
});

// Vi sparar databasen före och lägger tillbaka den efter
let dbBefore: DatabaseHehe[];
beforeAll(async () => {
  dbBefore = await knexInstance<DatabaseHehe>(HEHE_TABLE).select('*');
...
afterAll(async () => {
  ...
  if (dbBefore != null && dbBefore.length > 0) {
    await knexInstance<DatabaseHehe>(HEHE_TABLE).insert(dbBefore);
  }
});
...
test('getting all HeHEs without limit, ascending order', async () => {
  ...
  // Lägg till våra HeHE
  await knexInstance<DatabaseHehe>(HEHE_TABLE).insert([localHehe0, localHehe1, DUMMY_HEHE]);
...
test('getting single HeHE', async () => {
  await knexInstance<DatabaseHehe>(HEHE_TABLE).insert(DUMMY_HEHE);
