...
test('getting current holders of a post without holder should be empty list', async () => {
  const { id: postId } = await api.createPost(np);
  ...
  expect(holders[0].usernames).toHaveLength(1);
  expect(holders[0].usernames[0]).toEqual(dummyUser.username);
});
...
test('getting current holders for multiple posts', async () => {
  const [{ id: postId0 }, { id: postId1 }] = await Promise.all([
    api.createPost(np),
    api.createPost({ ...np, name: getRandomPostname() }),
  ]);

  // No holders means no response
  await expect(api.getCurrentPostHolders([postId0, postId1])).resolves.toHaveLength(0);

  const addUsers = [
    api.addUsersToPost([dummyUser.username], postId0),
    api.addUsersToPost([dummyUser.username], postId1),
  ];

  await expect(Promise.all(addUsers)).resolves.toBeTruthy();

  const holders = await api.getCurrentPostHolders([postId0, postId1]);
  expect(holders).toEqual(
    expect.arrayContaining([
      {
        postId: postId0,
        usernames: [dummyUser.username],
      },
      {
        postId: postId1,
        usernames: [dummyUser.username],
